  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await emailjs.sendForm(
        'service_jkzdcif', // Your EmailJS service ID
        'template_909r5yj', // Your EmailJS template ID
        form.current,
        'y9squEuihRVrao9ou' // Your EmailJS public key
      );
    ...
    <div className="container mx-auto p-6 rounded-lg shadow-lg max-w-2xl border border-gray-100 dark:border-gray-700 py-10 mt-20">
      ...
      <form ref={form} onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {['from_first_name', 'from_last_name'].map((name) => (
            <div className="flex flex-col" key={name}>
              <label className="text-gray-600 dark:text-gray-200 font-medium mb-2">
                {name === 'from_first_name' ? 'First Name' : 'Last Name'}
              </label>
              <input
                type="text"
                name={name}
                ...
                placeholder={`Enter your ${name === 'from_first_name' ? 'first' : 'last'} name`}
              />
            </div>
          ))}
        </div>
